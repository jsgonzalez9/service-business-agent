...
export async function POST(req: NextRequest) {
  ...
  const lines = [
    { text: "Assignment of Contract", x: 72, y: 740 },
    ...
    { text: "This document is a summary; the binding agreement is your standard assignment contract." },
  ]
  const pdfBuf = buildPdf(lines)
  const path = `assignments/${lead.id}-${Date.now()}.pdf`
  ...
  try {
    if (lead.phone_number) await sendSMS(lead.phone_number, `Assignment summary generated. Link: ${signed?.signedUrl || "[dashboard]"}`, { withFooter: false, bypassSuppression: true })
    if (buyer.phone) await sendSMS(buyer.phone, `Assignment summary for ${lead.address}. Link: ${signed?.signedUrl || "[dashboard]"}`, { withFooter: false, bypassSuppression: true })
  } catch {}
